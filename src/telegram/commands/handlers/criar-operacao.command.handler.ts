...
export class CriarOperacaoCommandHandler implements ITextCommandHandler, OnModuleDestroy {
  ...
  async handleCallback(ctx: any): Promise<boolean> {
    ...
    if (data.startsWith('op_type_')) {
      ...
      if (session) {
        ...
        // ✅ VALIDAÇÃO CRÍTICA: Verificar dados obrigatórios da sessão
        const missingFields: string[] = [];
        if (!session.data.type) missingFields.push("tipo");
        if (!session.data.assets || session.data.assets.length === 0) missingFields.push("ativos");
        if (!session.data.networks || session.data.networks.length === 0) missingFields.push("redes");
        if (!session.data.amount) missingFields.push("valor");
        if (!session.data.quotationType) missingFields.push("cotação");
        
        if (missingFields.length > 0) {
          this.logger.error(`❌ Dados incompletos na sessão: ${missingFields.join(", ")}`);
          await ctx.answerCbQuery(
            `⚠️ Dados incompletos! Faltando: ${missingFields.join(", ")}. Inicie nova operação.`,
            { show_alert: true }
          );
          this.sessions.delete(sessionKey);
          return true;
  ...
  async handleTextInput(ctx: TextCommandContext): Promise<void> {
    ...
    if (session.step === 'amount_quotation') {
      ...
      // FIX: Verificar se cotação e valor foram selecionados antes de ir pra descrição
      if (!session.data.quotationType || !session.data.amount) {
        await this.showValueInput(ctx);
      } else {
        await this.showDescriptionInput(ctx);
      }
